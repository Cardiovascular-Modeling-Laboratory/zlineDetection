function [Dxx,Dxy,Dyy]=ConstructDiffusionTensor2DLap(mu1,mu2,v1x,v1y,v2x,v2y,gradA,Options)
...
elseif(Options.eigenmode==5)
    di=(-mu1+abs(mu2)); di(di<1e-15)=1e-15; di(di>1e-15)=di(di>1e-15).^2;   % Use my Laplacian stuff
    % Implicit if mu1 == mu2 then lambda1=alpha
    lambda1 = Options.alpha + (1 - Options.alpha)*exp(-Options.C./(di).^(2*Options.m)); 
    lambda2 = Options.alpha;
end
